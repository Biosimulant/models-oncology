...
class Hutchinson2016VascularPhenotypeIdentificationModel1911130007Model(TelluriumSBMLBioModule):
    ...
    _PARAMETER_INPUTS = {}
    _INITIAL_CONDITION_INPUTS = {'epsilon_level': ('epsilon', 0.04, 'native SBML value', 'Epsilon source parameter. Maps to bundled SBML parameter `epsilon`.')}
    _HEADLINE_OUTPUTS = {'model_state_1': ('v', 'native SBML value', 'V observable. Maps to SBML symbol `v`.'), 'model_state_2': ('rv', 'native SBML value', 'Rv observable. Maps to SBML symbol `rv`.'), 'model_state_3': ('bv', 'native SBML value', 'Bv observable. Maps to SBML symbol `bv`.'), 'model_state_4': ('a1', 'native SBML value', 'A1 observable. Maps to SBML symbol `a1`.'), 'model_state_5': ('aw', 'native SBML value', 'Aw observable. Maps to SBML symbol `aw`.'), 'model_state_6': ('a2', 'native SBML value', 'A2 observable. Maps to SBML symbol `a2`.')}
