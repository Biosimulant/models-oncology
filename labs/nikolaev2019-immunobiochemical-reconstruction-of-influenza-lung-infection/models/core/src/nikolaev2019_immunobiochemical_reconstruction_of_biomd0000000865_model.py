...
class Nikolaev2019ImmunobiochemicalReconstructionOfBiomd0000000865Model(TelluriumSBMLBioModule):
    ...
    _PARAMETER_INPUTS = {}
    _INITIAL_CONDITION_INPUTS = {'sigma_p_tilde_source_parameter': ('sigma_p_tilde', 0.1, 'native SBML value', 'Sigma p tilde source parameter. Maps to bundled SBML parameter `sigma_p_tilde`.')}
    _HEADLINE_OUTPUTS = {'chemotherapy_level': ('C', 'native SBML value', 'C observable. Maps to SBML symbol `C`.'), 'p': ('P', 'native SBML value', 'P observable. Maps to SBML symbol `P`.'), 'infected_tumor_cells': ('I', 'native SBML value', 'I observable. Maps to SBML symbol `I`.'), 'b': ('B', 'native SBML value', 'B observable. Maps to SBML symbol `B`.')}
